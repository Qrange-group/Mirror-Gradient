...
class GCN(torch.nn.Module):
    def __init__(
        ...
        if self.dim_latent:
            ...
            self.g_layer1 = (
                nn.Linear(self.dim_latent + self.dim_id, self.dim_id)
                if self.concate
                else nn.Linear(self.dim_latent, self.dim_id)
            )
            ...
            self.conv_embed_1 = BaseModel(
                self.dim_feat, self.dim_feat, aggr=self.aggr_mode
            )
            nn.init.xavier_normal_(self.conv_embed_1.weight)
            ...
            self.g_layer1 = (
                ...
                if self.concate
                else nn.Linear(self.dim_feat, self.dim_id)
            )
            nn.init.xavier_normal_(self.g_layer1.weight)
    ...
    def forward(self, features, id_embedding):
        ...
        x_hat = (
            F.leaky_relu(self.linear_layer1(x)) + id_embedding
            if self.has_id
            else F.leaky_relu(self.linear_layer1(x))
        )  # equation 5
        x = (
            F.leaky_relu(self.g_layer1(torch.cat((h, x_hat), dim=1)))
            if self.concate
            else F.leaky_relu(self.g_layer1(h) + x_hat)
        )

        h = F.leaky_relu(self.conv_embed_2(x, self.edge_index))  # equation 1
        x_hat = (
            F.leaky_relu(self.linear_layer2(x)) + id_embedding
            if self.has_id
            else F.leaky_relu(self.linear_layer2(x))
        )  # equation 5
        x = (
            F.leaky_relu(self.g_layer2(torch.cat((h, x_hat), dim=1)))
            if self.concate
            else F.leaky_relu(self.g_layer2(h) + x_hat)
        )

        h = F.leaky_relu(self.conv_embed_3(x, self.edge_index))  # equation 1
        x_hat = (
            F.leaky_relu(self.linear_layer3(x)) + id_embedding
            if self.has_id
            else F.leaky_relu(self.linear_layer3(x))
        )  # equation 5
        x = (
            F.leaky_relu(self.g_layer3(torch.cat((h, x_hat), dim=1)))
            if self.concate
            else F.leaky_relu(self.g_layer3(h) + x_hat)
        )
