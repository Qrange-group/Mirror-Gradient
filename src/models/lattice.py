...
class LATTICE(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        image_adj_file = os.path.join(
            dataset_path, "image_adj_{}.pt".format(self.knn_k)
        )
    ...
    def forward(self, adj, build_item_graph=False):
        ...
        if build_item_graph:
            ...
            if self.v_feat is not None:
                ...
                self.image_adj = build_knn_neighbourhood(
                    self.image_adj, topk=self.knn_k
                )
            ...
            self.item_adj = (
                1 - self.lambda_coeff
            ) * learned_adj + self.lambda_coeff * original_adj
        else:
        ...
        if self.cf_model == "ngcf":
            ...
            return (
                ...
                self.item_id_embedding.weight + F.normalize(h, p=2, dim=1),
            )
    ...
    def calculate_loss(self, interaction):
        ...
        ua_embeddings, ia_embeddings = self.forward(
            self.norm_adj, build_item_graph=self.build_item_graph
        )
    ...
    def full_sort_predict(self, interaction):
        ...
        restore_user_e, restore_item_e = self.forward(
            self.norm_adj, build_item_graph=True
        )
